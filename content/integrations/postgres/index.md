...
description: >
  Protect your PostgreSQL databases against data loss, accidental deletion, and
  misconfiguration. Plakar provides encrypted, deduplicated snapshots using either
  logical or physical backups, restorable to any environment.

technology_title: PostgreSQL is battle-tested, but still need a backup plan

technology_description: >
  PostgreSQL is one of the most trusted databases in production, but durability
  at the database level is not the same as having a reliable backup strategy.
  Replication spreads data across nodes which also spreads corruption and accidental
  deletions just as quickly.

  Plakar integrates with PostgreSQL's own tooling (pg_dump and pg_basebackup)
  adding encryption, deduplication, and snapshot management on top of the backup
