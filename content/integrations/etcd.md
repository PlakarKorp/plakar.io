...
technology_description: >
  etcd is a distributed key-value store used as the source of truth in many distributed systems, most notably Kubernetes. It is designed to survive partial node failures, but it cannot recover on its own if too many nodes fail simultaneously.

  Without an independent backup, a wide cluster failure means permanent loss of all cluster state. Plakar fills that gap by storing encrypted, immutable snapshots of etcd outside the cluster itself.
