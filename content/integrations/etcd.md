...
technology_description: >
  ...
  distributed systems, most notably Kubernetes. It is designed to survive
  partial node failures, but it cannot recover on its own if too many nodes fail
  simultaneously.

  Without an independent backup, a wide cluster failure means permanent loss of
  all cluster state. Plakar fills that gap by storing encrypted, immutable
