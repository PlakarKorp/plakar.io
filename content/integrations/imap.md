...
description: >
  Back up and restore IMAP mailboxes with end-to-end encryption, immutability,
  and verifiable integrity. Plakar ensures your email history remains protected
  across any IMAP-compatible mail server.

technology_title: IMAP powers nearly all email clients and servers

technology_description: >
  IMAP (Internet Message Access Protocol) is the standard protocol for accessing
  email across virtually all mail servers and providers, from corporate Exchange
  servers to personal Gmail accounts. While IMAP provides synchronized access to
  your mail, it does not provide immutability, versioning, or protection against
  accidental deletion or account compromise. Plakar fills that gap by turning
  any IMAP-accessible mailbox into a secure backup source or restoration target,
  with encrypted, deduplicated, and verifiable snapshots independent of your
  mail server's state.
