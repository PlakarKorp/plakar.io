...
subtitle: Discovery, secrets, and encrypted backups for OVHcloud Public Cloud

description: >
  Manage OVHcloud Public Cloud from Plakar Control Plane end to end. Discover
  compute instances, block storage volumes, and object storage automatically,
  manage secrets through OVHcloud's own secret store, and protect resources with
  encrypted, deduplicated backups and restores.

technology_title: Public Cloud infrastructure is easy to spin up, easy to lose

technology_description: >
  OVHcloud Public Cloud makes it fast to provision compute instances, block
  storage volumes, and object storage, but speed cuts both ways: resources
  created quickly can be deleted, corrupted, or compromised just as quickly, and
  the platform itself has no built-in way to step outside itself and keep an
  independent copy safe. Plakar adds an inventory, a secret provider, an
  exporter, and an importer for OVHcloud Public Cloud, so resources can be
  discovered, their access secrets managed centrally, and their data captured,
  stored in a Kloset, and restored back to the same project or a new one.
...
links:
  - type: control-plane
    url: /docs/control-plane/resources/compute/ovhcloud/
