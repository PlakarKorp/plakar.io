...
subtitle: Encrypted, verifiable backups for VMware virtual machines

description: >
  Protect virtual machines running on vCenter, ESXi, or a vSphere-based cloud
  platform against accidental deletion, ransomware, and infrastructure failure.
  Plakar discovers, backs up, and restores VMware workloads with independent,
  encrypted snapshots.

technology_title:
  Virtualization protects against hardware failure, not human or malicious error

technology_description: >
  VMware vSphere is built to keep virtual machines running through hardware
  failure, with features like live migration and high availability doing the
  heavy lifting. What vSphere doesn't provide on its own is an independent
  history you can go back to when the problem isn't the hardware: a VM that's
  deleted by mistake, encrypted by ransomware, or corrupted by a bad update.
  Plakar adds that history, discovering VMware environments, capturing encrypted
  and deduplicated backups of the virtual machines running in them, and
  restoring those machines to the same environment or a new one whenever they're
  needed.
...
image: img/integrations/vmware.svg
---
