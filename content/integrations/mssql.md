...
subtitle: Encrypted, application-consistent backups for SQL Server on Windows

description: >
  Protect Microsoft SQL Server against accidental deletion, ransomware, and
  failed maintenance without taking the database offline to back it up. Plakar
  captures a consistent snapshot of a live SQL Server installation and restores
  it with confidence, on the original server or a new one.

technology_title: SQL Server keeps your data organized, not backed up

technology_description: >
  Microsoft SQL Server is built to store and serve business-critical data
  reliably while it's running, but it has no built-in way to step outside itself
  and keep an independent copy safe. Anything that threatens the server, from a
  bad update to a ransomware attack to a simple mistake, puts both the live
  database and any backups sitting next to it at risk. Plakar captures a
  consistent, point-in-time copy of a SQL Server installation while it's healthy
  and online, encrypts it, and stores it independently, so a working recovery
  point exists no matter what happens to the server itself.
...
seo_tags:
  ...
  - SQL Server ransomware protection
  - encrypted database backup
  - Windows Server database backup
  - SQL Server recovery
...
image: img/integrations/mssql.png
---
