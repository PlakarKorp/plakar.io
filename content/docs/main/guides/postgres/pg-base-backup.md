...
`pg_basebackup` is the standard PostgreSQL utility used to perform **physical backups** of a PostgreSQL cluster. Unlike SQL dumps, a physical backup captures the entire data directory, allowing you to restore a PostgreSQL instance exactly as it was at backup time, including configuration files and internal state.
...
It is possible to use other store locations, for example to store the snapshots in a cloud storage bucket. Check the guide [Creating a Kloset Store](../../create-kloset-repository) for more information on setting up Kloset stores.
