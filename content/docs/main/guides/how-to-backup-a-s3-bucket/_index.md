...
Installing the pre-built packages hosted on Plakar's infrastructure requires you to be logged in. See [Unlocking Features with plakar login](../what-is-plakar-login) to learn how to log in.
...
```bash
plakar pkg build s3
plakar pkg add ./s3_vx.y.z_darwin_arm64.ptar
```
