...
summary:
  ...
  help from the CLI."
aliases:
  - /docs/v1.0.6/references/command-line-syntax/
  - /docs/v1.0.6/guides/plakar-command-line-syntax
---
...
When creating or opening an encrypted repository, Plakar prompts for a
passphrase. Setting `PLAKAR_PASSPHRASE` provides it automatically, which is
useful in scripts, CI pipelines, or any non-interactive context where a terminal
prompt isn't available.

### `PLAKAR_REPOSITORY`

Sets the default repository location so you don't need to specify
`at REPOSITORY` on every command. When omitted and no `at` clause is provided,
Plakar falls back to `~/.plakar`.
