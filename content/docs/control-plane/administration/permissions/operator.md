...
weight: 6
summary:
  "Runs and defines the backup chain, without administering the organization."
---
...
chain a backup runs through. The remaining resource classes are held at the
configure level, so it can define and run the work but cannot delete those
...
must be kept. Like every role except the [Superuser](../superuser) and the
[Platform Admin](../platform-admin), it holds nothing in the deployment family.
