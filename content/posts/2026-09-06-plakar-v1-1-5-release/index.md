---
title: "Plakar v1.1.5 is out"
date: 2026-09-06T12:00:00+0100
authors:
  - "gilles"
summary:
  "plakar v1.1.5 is out: a security-focused patch release. Unencrypted stores
  are now refused unless you explicitly opt in, plugins are signature-checked at
  install time, and the HTTP store and server got hardened. Fully backward
  compatible, nothing to migrate."
categories:
  - announcement
  - release-notes
  - community
...
unencrypted stores, and until now it would quietly work with one if that is what
...
still work, for testing, debugging, or the rare setup that genuinely wants one,
but you now have to say so, out loud, every time. A misconfiguration becomes a
loud failure.

If you run a plaintext store on purpose, add the variable to your environment
and carry on. Everyone else: this change is invisible, exactly as it should be.
...
As of v1.1.5, **packages are signature-checked at install time**.
`plakar pkg add` verifies the signature before anything lands on disk, and
refuses unsigned packages by default. If you are developing an integration or
have a reason to trust an unsigned package, `plakar pkg add -allow-unsigned`
opts out, explicitly, and only for that install.

The new `-devel` flag on `plakar pkg add` and `plakar pkg show` opts into the
devel tree, where integrations that are not yet stable are published. These
packages are signed too.

## Also in this release

More hardening in the same spirit:

- `plakar server` accepts `-token` to require an Authorization Bearer token from
  clients, and now logs requests.
